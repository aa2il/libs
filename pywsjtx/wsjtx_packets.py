...
class HeartBeatPacket(GenericWSJTXPacket):
    ...
    # JBA - print formatting
    def __repr__(self):
        str ='HeartBeat:\t'
        str+='\twsjtx id:{}\tmax_schema:{}\tschema:{}\tversion:{}\trevision:{}' \
            .format(self.wsjtx_id, self.max_schema, self.schema, self.version, self.revision)
        return str
                                                                                                            # OLD
...
class StatusPacket(GenericWSJTXPacket):
    ...
    # JBA - This is suppose to return the differences
    ...
    # JBA - this is how the printing is formatted
    def __repr__(self):
        str =  'Status:\t'
        str += "Freq:{}\trx_df:{}\ttx_df:{}\tdx_call:{}\tdx_grid:{}\treport:{}\n". \
               format(self.dial_frequency, self.rx_df, self.tx_df, self.dx_call, self.dx_grid, self.report)
        str += "\tTx:{}\tDecoding:{}\tTxEnable:{}\tTxWatchDog:{}\tSubMode:{}\tFastMode:{}\tSpecial_op_mode:{}". \
               format(self.transmitting, self.decoding, self.tx_enabled, self.tx_watchdog, \
...
class DecodePacket(GenericWSJTXPacket):
    ...
    def format_spot(self,old_status,my_call):
        ...
        wsjt_modes={'~':'FT8', '+':'FT4', '$':'JT4', '@':'JT9', '#':'JT65', \
                    ':':'QRA64', ' ':'ISCAT', '&':'MSK144' }
        mode=wsjt_modes[self.mode]
